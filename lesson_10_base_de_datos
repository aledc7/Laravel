...
Al realizar una modificación en la migración original tenemos el problema de que los comandos reset y refresh __eliminarán el contenido de las tablas__ en la base de datos. Para evitar esto podemos (utilizando el comando make:migration) crear una nueva migración y agregar desde ahí las modificaciones que necesitamos:
...
Con el método __dropColumn__ eliminamos de la tabla la columna especificada como argumento.

## Indicar la posición de una columna
Al modificar una migración utilizando __Schema::table__ y agregar una columna, esta se va añadir al final de la tabla. Podemos lograr que la columna se cree en la posición que indiquemos utilizando el método after:
...
## Comando rollback
Utilizando el camando de Artisan __migrate:rollback__ Laravel regresará el último lote de migraciones ejecutado:
...
[Ir a la Siguiente Leccion - Modificar Tablas ->](https://github.com/aledc7/Laravel/blob/master/lesson_11_modificar_tablas.md)
